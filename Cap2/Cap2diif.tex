...
%\minitoc
  ...
  los cuales, en general, est\'an en conflicto unos con otros al mismo tiempo. En \DIFdelbegin \DIFdel{este cap\'itulo }\DIFdelend \DIFaddbegin \DIFadd{esta secci\'on }\DIFaddend se describen algunos 
  conceptos b\'asicos de algoritmos evolutivos, de la optimizaci\'on \DIFdelbegin \DIFdel{multi-objetivo }\DIFdelend \DIFaddbegin \DIFadd{multiobjetivo }\DIFaddend y del algoritmo de c\'umulos de 
  ...
  \begin{itemize}
   ...
   De acuerdo con el \textit{neodarwinismo}, se tienen cuatro procesos estoc\'asticos que actu\'an en todas las especies\DIFdelbegin %DIFDELCMD < \cite{hoffman89}%%%
\DIFdelend :
   \begin{itemize}
  ...
  \begin{figure}[H]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.45]{Cap2/1-16.eps}
%DIFDELCMD < 	  %%%
...
\section{Optimizaci\'on multi-objetivo}
      ...
      Los problemas \DIFdelbegin \DIFdel{multi-objetivo }\DIFdelend \DIFaddbegin \DIFadd{multiobjetivo }\DIFaddend son aquellos en los que se deben optimizar $k \geq 2$ funciones objetivo simult\'aneamente. 
	  ...
	  Un \DIFdelbegin \DIFdel{problema de optimizaci\'on multi-objetivo }\DIFdelend \DIFaddbegin \DIFadd{Problema de Optimizaci\'on Multiobjetivo }\DIFaddend se define como la tarea de minimizar (o maximizar) 

	  \[F\left(\vec{x}\right)=\left(f_1\left(\vec{x}\right), \ldots,f_k\left(\vec{x}\right) \right)\DIFdelbegin \DIFdel{,}\DIFdelend \]

	  \DIFdelbegin %DIFDELCMD < {\setlength{\parindent}{0pt}
%DIFDELCMD < 	  
...
\DIFdelend \DIFaddbegin 
      ...
      $\vec{y} = \left(y_1 , \ldots, y_k \right)$ \DIFdelbegin \DIFdel{. La }\DIFdelend \DIFaddbegin \DIFadd{(ver }\DIFaddend figura ~\ref{fig:multiobj}\DIFdelbegin \DIFdel{muestra un problema de dos objetivos con dos variables}\DIFdelend \DIFaddbegin \DIFadd{)}\DIFaddend .

      \begin{figure}
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.6]{Cap2/1-1.eps}
%DIFDELCMD < 	  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.65]{Cap2/1-1.eps}
	  \DIFaddendFL \caption{Un problema de dos objetivos con dos variables}
      \label{fig:multiobj}
      \end{figure}

      Cuando el problema es \DIFdelbegin \DIFdel{multi-objetivo }\DIFdelend \DIFaddbegin \DIFadd{multiobjetivo }\DIFaddend existe un conflicto que ocasiona que la mejora en alg\'un objetivo provoque 
      el deterioro de otros. Por lo tanto, un problema de optimizaci\'on \DIFdelbegin \DIFdel{multi-objetivo }\DIFdelend \DIFaddbegin \DIFadd{multiobjetivo }\DIFaddend consiste en encontrar el mejor 
	  ...
	  \DIFaddbegin \DIFadd{($A$ y $B$ son }\'optimos \DIFadd{de Pareto)}\DIFaddend .
      \end{definicion}

      La figura \ref{fig:paretodominance} muestra gr\'aficamente la dominancia de Pareto para un problema de optimizaci\'on
      \DIFdelbegin \DIFdel{con dos objetivos }\DIFdelend \DIFaddbegin \DIFadd{bi-objetivo }\DIFaddend en el cual $A \preceq C$ tal que $A$ es mejor en ambas funciones, $A \preceq E$ tal que $A$ es igual con respecto 
      a $f_2$ pero es mejor con respecto a $f_1$, $B \preceq E$ tal que $B$  es igual con respecto a $f_1$ pero mejor con respecto a 
      $f_2$, $B\preceq D$ tal que B es mejor en ambas funciones, y $A$ y $B$ son incomparables\DIFdelbegin \DIFdel{($A$ y $B$ son }%DIFDELCMD < \'optimos %%%
\DIFdel{de Pareto)}\DIFdelend .

       \begin{figure}
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.6]{Cap2/1-4.eps}
%DIFDELCMD < 	  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.65]{Cap2/1-4.eps}
	  \DIFaddendFL \caption{Dominancia de Pareto}
      \label{fig:paretodominance}
      \end{figure}

      \begin{definicion}[Conjunto de \'optimos de Pareto]
	Para un problema \DIFdelbegin \DIFdel{multi-objetivo }\DIFdelend \DIFaddbegin \DIFadd{multiobjetivo }\DIFaddend determinado $F\left(\vec{x}\right)$, el conjunto de \'optimos de Pareto, denotado por $P^*$ 
	...
	P^*= \left\{ \vec{x} \in \Psi |\hspace{0.3cm} \nexists \vec{y}\in \Psi \hspace{0.7cm} F\left(\vec{y}\right)\preceq F\left(\vec{x}\right) \right\}\]
      ...
      Para un problema \DIFdelbegin \DIFdel{multi-objetivo }\DIFdelend \DIFaddbegin \DIFadd{multiobjetivo }\DIFaddend determinado $F\left(\vec{x} \right)$, siendo su conjunto de \'optimos de Pareto\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend $P^*$ \DIFdelbegin \DIFdel{) }\DIFdelend , el Frente 
	...
	\includegraphics[scale=0.65]{Cap2/1-2.eps}
      ...
      Para un problema \DIFdelbegin \DIFdel{multi-objetivo }\DIFdelend \DIFaddbegin \DIFadd{multiobjetivo }\DIFaddend determinado $F\left(\vec{x}\right)$, siendo su conjunto de \'optimos de Pareto, $P^*$, 
      el vector ideal se define como (ver figura \ref{fig:ideanadir}):

      \[  f_{ideal}= \left(^{\min}_{\DIFdelbegin %DIFDELCMD < \vec{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend \in P\DIFdelbegin \DIFdel{^*}\DIFdelend }f_1\left(\DIFdelbegin %DIFDELCMD < \vec{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend \right), \ldots, ^{\min}_{\DIFdelbegin %DIFDELCMD < \vec{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend \in P\DIFdelbegin \DIFdel{^*}\DIFdelend }f_k\left(\DIFdelbegin %DIFDELCMD < \vec{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend \right) \right)\]
      ...
      \[  f_{nadir}= \left(^{\max}_{\DIFdelbegin %DIFDELCMD < \vec{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend \in P\DIFdelbegin \DIFdel{^*}\DIFdelend }f_1\left(\DIFdelbegin %DIFDELCMD < \vec{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend \right), \ldots, ^{\max}_{\DIFdelbegin %DIFDELCMD < \vec{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend \in P\DIFdelbegin \DIFdel{^*}\DIFdelend }f_k\left(\DIFdelbegin %DIFDELCMD < \vec{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend \right) \right)\]
...
\DIFdelend \DIFaddbegin 
      ...
      $IGD = 0$ indica que el frente obtenido es el verdadero frente de Pareto\DIFdelbegin \DIFdel{, cualquier otro valor indica que el frente obtenido 
...
\DIFdelend \DIFaddbegin \DIFadd{.
      ...
      \DIFdelbegin %DIFDELCMD < \begin{definicion}[Cobertura de Conjuntos ($\mathcal{C}$)]
%DIFDELCMD < 	  %%%
\DIFdelend \DIFaddbegin \begin{definicion}[Cobertura (Two Set Coverage) (CS)]
	  \DIFaddend Determina la cobertura relativa de un conjunto, con respecto a otro. Fue propuesta por \cite{Zitzler2000} para 
	  valorar cuantitativamente cu\'anto un conjunto cubre o domina a otro. \DIFdelbegin \DIFdel{Si $A$ y $B$ son dos conjuntos no dominados,
	  la m\'etrica de 
	  cobertura de $\mathcal{C}(A,B)$ calcula la fracci\'on de vectores en $B$ que son dominados 
	  d\'ebilmente por los vectores de $A$}\DIFdelend \DIFaddbegin \DIFadd{Sean $X \ e \ Y \subseteq X$, dos conjuntos de 
	  vectores de decisi\'on (frentes), $CS$ se define formalmente como la transformaci\'on de ($X$, $Y$) al rango 
	  $[0,1]$}\DIFaddend . Se define como:

	  \[\DIFdelbegin \DIFdel{\mathcal{C}}\DIFdelend \DIFaddbegin \DIFadd{CS}\DIFaddend \left(\DIFdelbegin \DIFdel{A, B }\DIFdelend \DIFaddbegin \DIFadd{X, Y }\DIFaddend \right) = \DIFdelbegin \DIFdel{\frac{|\vec{b} \in B; \exists \vec{a} \in A: \vec{a}\preceq \vec{b}|}{|B|}
	  }\DIFdelend \DIFaddbegin \DIFadd{\frac{|b \in Y; \exists a \in X: a\preceq b|}{|Y|}}\DIFaddend \]

	  Si todos los \DIFdelbegin \DIFdel{vectores en $B$ }\DIFdelend \DIFaddbegin \DIFadd{puntos en $Y$ }\DIFaddend son dominados o son iguales a \DIFdelbegin \DIFdel{los vectores en $A$, entonces $\mathcal{C}$ }\DIFdelend \DIFaddbegin \DIFadd{puntos de $X$, entonces $CS$ }\DIFaddend es igual a uno. Si ninguno de los 
	  \DIFdelbegin \DIFdel{vectores en $B$ }\DIFdelend \DIFaddbegin \DIFadd{puntos en $Y$ }\DIFaddend es dominado o igual a alg\'un \DIFdelbegin \DIFdel{vector de $A$, entonces $\mathcal{C}$ }\DIFdelend \DIFaddbegin \DIFadd{punto de $X$, $CS$ }\DIFaddend es igual a cero. Lo ideal es que \DIFdelbegin \DIFdel{$\mathcal{C} \left(A, B\right)$  y $\mathcal{C} \left(B, A \right)$ }\DIFdelend \DIFaddbegin \DIFadd{$CS \left(X, Y\right)$ 
	  y $CS \left(Y, X \right)$ }\DIFaddend se consideren de manera separada, ya que \DIFdelbegin \DIFdel{$\mathcal{C}\left(A, B\right)$ }\DIFdelend \DIFaddbegin \DIFadd{$CS \left(X, Y\right)$ }\DIFaddend no es equivalente a
	  \DIFdelbegin \DIFdel{$1- \mathcal{C}\left(B, A\right)$
      }\DIFdelend \DIFaddbegin \DIFadd{$1- CS \left(X, Y\right)$
      }\DIFaddend \end{definicion}

      \DIFdelbegin %DIFDELCMD < \begin{definicion}[Dispersi\'on o Espaciamiento (Spread) (S)]
%DIFDELCMD < 	 %%%
\DIFdelend \DIFaddbegin \begin{definicion}[Dispersi\'on (Spread) (S)]
  ...
  multi-objetivo por primera vez, pero no lleg\'o a desarrollar un algoritmo, dado que \DIFdelbegin \DIFdel{transform\'o }\DIFdelend \DIFaddbegin \DIFadd{adopt\'o por transformar }\DIFaddend el problema 
  bi-objetivo de su inter\'es en uno mono-objetivo \cite{Rosenberg67}. \DIFdelbegin \DIFdel{La }\DIFdelend \DIFaddbegin \cite{Schaffer84} \DIFadd{propuso la }\DIFaddend primera implementaci\'on de lo que 
  actualmente se conoce como un algoritmo evolutivo multi-objetivo\DIFdelbegin \DIFdel{fue propueta por }%DIFDELCMD < \cite{Schaffer84}%%%
\DIFdelend . 
  ...
  encontrar varias soluciones del conjunto \DIFdelbegin \DIFdel{de }\DIFdelend \'optimos de Pareto en una sola ejecuci\'on. Tambi\'en, son menos sensibles a la forma o 
  continuidad del frente de Pareto. Adicionalmente, son f\'aciles de usar y \DIFdelbegin \DIFdel{no }\DIFdelend requieren informaci\'on espec\'ifica del problema a resolverse.
  ...
  con un estimador densidad\DIFdelbegin \DIFdel{en el espacio de las funciones objetivo que sesga el mecasnimo de selecci\'on de manera 
  que se mantenga diversidad en la poblaci\'on}\DIFdelend . Sin embargo, el operador de selecci\'on espera un solo valor de aptitud. La forma m\'as sencilla de mantener 
  ...
  \begin{algorithm}
      \begin{algorithmic}[1]			
	\STATE \DIFdelbegin \DIFdel{$t \leftarrow 0$}\DIFdelend \DIFaddbegin \DIFadd{$t=0$}\DIFaddend .
	\STATE Generar aleatoriamente la poblaci\'on inicial $P^t$.
	\DIFdelbegin %DIFDELCMD < \WHILE{$t < max_{Gen}$}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \WHILE{Condici\'on de paro}
	\DIFaddend \STATE Para cada individuo \DIFdelbegin \DIFdel{$\vec{x} \in P^t$ }\DIFdelend \DIFaddbegin \DIFadd{$x \in P^t$ }\DIFaddend calcular el valor escalar de aptitud \DIFdelbegin \DIFdel{$F(\vec{x})$}\DIFdelend \DIFaddbegin \DIFadd{$F(x)$}\DIFaddend .
	\STATE Seleccionar de $P^t$ un grupo de padres $P'^t$ bas\'andose en la aptitud \DIFaddbegin \DIFadd{$F(x)$}\DIFaddend .
	\STATE Recombinar los individuos de $P'^t$ para obtener $P''^t.$
	\STATE Mutar los individuos de $P''(t)$ para obtener $P'''^t$.
	\STATE \DIFdelbegin \DIFdel{$P^{t+1 } \leftarrow P'''^t$}\DIFdelend \DIFaddbegin \DIFadd{$P^{t+1 } = P'''^t$}\DIFaddend .
	\STATE \DIFdelbegin \DIFdel{$t\leftarrow t +1$}\DIFdelend \DIFaddbegin \DIFadd{$t=t+1$}\DIFaddend .
	\ENDWHILE 
  \end{algorithmic}
  ...
  \DIFdelbegin \DIFdel{La primera,
  }\DIFdelend \DIFaddbegin \DIFadd{Una de ellos }\DIFaddend es combinar la poblaci\'on \DIFdelbegin \DIFdel{anterior }\DIFdelend \DIFaddbegin \DIFadd{vieja }\DIFaddend y la descendencia\DIFdelbegin \DIFdel{o la poblaci\'on nueva}\DIFdelend , y posteriormente aplicar una selecci\'on determinista 
  en lugar de reemplazar la \DIFdelbegin \DIFdel{vieja }\DIFdelend poblaci\'on \DIFaddbegin \DIFadd{vieja }\DIFaddend por la descendencia \cite{deb02}. \DIFdelbegin \DIFdel{La segunda, }\DIFdelend \DIFaddbegin \DIFadd{Otro enfoque }\DIFaddend es mantener una poblaci\'on secundaria 
  ...
  dominadas encontradas en el transcurso del proceso de b\'usqueda \cite{Zitzler99}(\DIFdelbegin \DIFdel{ver }\DIFdelend figura \ref{fig:elitismo}). En muchos problemas 
  ...
  \begin{itemize}
   ...
   incorporan el concepto de Pareto en su mecanismo de selecci\'on. \DIFdelbegin \DIFdel{Los }\DIFdelend \DIFaddbegin \DIFadd{Por ejemplo,  los }\DIFaddend siguentes algoritmos evolutivos son un \DIFdelbegin \DIFdel{conjunto representativo 
   que utilizan }\DIFdelend \DIFaddbegin \DIFadd{ejemplo 
   que proponen }\DIFaddend este enfoque en los \'ultimos a\~nos:
   ...
   \begin{itemize}
    ...
    enfoque de mantener diversidad, que consiste de un procedimiento de agrupamiento que divide el espacio de las funciones \DIFdelbegin \DIFdel{objetivo 
    }\DIFdelend \DIFaddbegin \DIFadd{bjetivo 
    ...
    Dado que el procedimiento es adaptivo, no se requieren \DIFdelbegin \DIFdel{par\'ametros }\DIFdelend \DIFaddbegin \DIFadd{parametros }\DIFaddend extras en el algoritmo \cite{paes99}.
    ...
    aptitud \DIFdelbegin \DIFdel{el }\DIFdelend \DIFaddbegin \DIFadd{la }\DIFaddend cual es proporcional al tama\~no de la poblaci\'on, para ofrecer el potencial reproductivo de estos 
    ...
    la densidad de la regi\'on. \DIFdelbegin \DIFdel{Por lo que, este }\DIFdelend \DIFaddbegin \DIFadd{Este }\DIFaddend algoritmo es m\'as eficiente y efectivo que su predecesor.

    \item \textbf{SPEA (\textit{The Strength Pareto Evolutionary Algorithm})}: este m\'etodo introduce elitismo al almacenar a los
    individuos no dominados en un archivo. Para cada individuo de este conjunto externo se calcula un valor \DIFdelbegin \DIFdel{llamado }\DIFdelend \DIFaddbegin \DIFadd{llamdo }\DIFaddend fortaleza 
    ...
    a altas probabilidades \DIFaddbegin \DIFadd{posibilidades }\DIFaddend de reproducci\'on. El esquema para asignar la aptitud pretende conseguir que la b\'usqueda 
...
\end{itemize}
  ...
  En este trabajo de investigaci\'on se plantea comparar el desempe\~no del algoritmo \DIFdelbegin \DIFdel{propuesto }\DIFdelend con respecto a las soluciones obtenidas con 
  \DIFdelbegin \DIFdel{los siguientes }\DIFdelend \DIFaddbegin \DIFadd{diferentes }\DIFaddend algoritmos de la literatura especializa\DIFdelbegin \DIFdel{:
  }\DIFdelend \DIFaddbegin \DIFadd{. 
  }\DIFaddend 

  \begin{itemize}
   \item El primero \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{de dichos algoritmos }\DIFaddend es el que presentan \cite{Raquel2005}\DIFaddbegin \DIFadd{, }\DIFaddend en el cual se 
  plantea el uso de \DIFdelbegin \DIFdel{un }\DIFdelend algoritmo de c\'umulos de part\'iculas para problemas multi-objetivo (\textit{An effective use of Crowding Distance 
  ...
  en el archivo \DIFdelbegin \DIFdel{externo}\DIFdelend \DIFaddbegin \DIFadd{exteno}\DIFaddend . El algoritmo \ref{alg:MOPSOcd} muestra la estructura b\'asica del MOPSOcd.
...
\begin{algorithm}
	...
	\ENSURE Soluciones no dominadas en un archivo acotado $A$.	  	  
	  \STATE Inicializar la velocidad \DIFdelbegin \DIFdel{$\vec{v} \leftarrow 0$ de cada part\'icula }\DIFdelend \DIFaddbegin \DIFadd{$v=0$ }\DIFaddend del c\'umulo $S$.
	  ...
	  \STATE Seleccionar el \DIFdelbegin \DIFdel{$\vec{pBest}$ }\DIFdelend \DIFaddbegin \DIFadd{$pBest$ }\DIFaddend inicial de las part\'iculas.
	  \STATE Seleccionar el \DIFdelbegin \DIFdel{$\vec{gBest}$ }\DIFdelend \DIFaddbegin \DIFadd{$gBest$ }\DIFaddend como el mejor encontrado en el c\'umulo $S$.
	  \STATE Inicializar el contador de generaciones con \DIFdelbegin \DIFdel{$t \leftarrow 0$}\DIFdelend \DIFaddbegin \DIFadd{$t=0$}\DIFaddend .
	...
	\WHILE {$t < max_{Gen}$}
	  ...
	  \FOR {\DIFdelbegin \DIFdel{$i\leftarrow 1$ }\DIFdelend \DIFaddbegin \DIFadd{$i=1$ }\DIFaddend hasta $tam_S$}
	      \STATE Seleccionar aleatoriamente el \DIFdelbegin \DIFdel{$\vec{gBest}$ }\DIFdelend \DIFaddbegin \DIFadd{$gBest$ }\DIFaddend desde una porci\'on especificada (por ejemplo, $10\%$ de la parte superior) 
	      del archivo ordenado $A$.
	     \STATE Generar una nueva velocidad 
		\\  \DIFdelbegin \DIFdel{$\vec{v}^{t+1}_{i} \leftarrow \omega \cdot \vec{v}^t_{i} + \phi_1 \cdot rnd_1 \cdot \left(\vec{pBes}t^t_i - \vec{S}^t_{i} \right) 
					    + \phi_2 \cdot rnd_2 \cdot \left(\vec{gBest} - \vec{S}^t_{i} \right)$	
	      }\DIFdelend \DIFaddbegin \DIFadd{$v^{t+1}_{i,k} = \omega \cdot v^t_{i,k} + \phi_1 \cdot rnd_1 \cdot \left(pBest^t_i - S^t_{i,k} \right) 
					    + \phi_2 \cdot rnd_2 \cdot \left(gBest - S^t_{i,k} \right)$	
	      }\DIFaddend \STATE Calcular las nuevas posiciones 
		\\\DIFdelbegin \DIFdel{$\vec{S}^{t+1}_{i} \leftarrow \vec{S}^{t}_{i}+\vec{v}^{t+1}_{i}$
	      }\DIFdelend \DIFaddbegin \DIFadd{$S^{t+1}_{i,k}=S^{t}_{i,k}+v^{t+1}_{i,k}$
	  ...
	  es actualizada utilizando \DIFdelbegin \DIFdel{$pBest_i \leftarrow S_i$
	  }\DIFdelend \DIFaddbegin \DIFadd{$pBest_i = S_i$
	  }\DIFaddend \STATE \DIFdelbegin \DIFdel{$t \leftarrow t + 1$}\DIFdelend \DIFaddbegin \DIFadd{$t = t + 1$}\DIFaddend .
	\ENDWHILE
      \end{algorithmic}
  ...
  c\'alculo de la contribuci\'on al hipervolumen que puede tomar mucho tiempo conforme se aumenta el n\'umero de objetivos.
   ...
   Los pilares principales que \DIFaddbegin \DIFadd{que }\DIFaddend enmarcan al algoritmo SMS-EMOA son:
  ...
  \begin{algorithm}
	...
	\STATE \DIFdelbegin \DIFdel{$t \leftarrow 0$
	}\DIFdelend \DIFaddbegin \DIFadd{$t=0$
  ...
  \begin{algorithm}
	...
	\REQUIRE Problema de Optimizaci\'on.
	\ENSURE Una poblaci\'on evolucionada.
	\STATE \DIFdelbegin \DIFdel{$t\leftarrow0$
	}\DIFdelend \DIFaddbegin \DIFadd{$t=0$
	...
	\WHILE {$t < max_{Gen}$}
	    ...
	    \STATE \DIFdelbegin \DIFdel{$R_t \leftarrow P_t \cup Q_t$
	    }\DIFdelend \DIFaddbegin \DIFadd{$R_t = P_t \cup Q_t$
...
\section{Algoritmo basado en c\'umulos de part\'iculas}
  ...
  El algoritmo de optimizaci\'on mediante c\'umulos de part\'iculas \DIFaddbegin \DIFadd{o }\DIFaddend fue propuesto en 1995 \DIFdelbegin \DIFdel{us\'andose }\DIFdelend \DIFaddbegin \DIFadd{usa\'andose }\DIFaddend originalmente
  para el balance de pesos de una red neuronal. A continuaci\'on se describen las principales componentes de la heur\'istica, 
  los modelos mono-objetivo m\'as caracter\'isticos y, finalmente, como llevar \DIFdelbegin %DIFDELCMD < \'esta %%%
\DIFdelend \DIFaddbegin \DIFadd{esta }\DIFaddend misma a su versi\'on multi-objetivo.
  ...
  con m\'as \'exito de su entorno. Con el tiempo, surge una cultura \DIFdelbegin \DIFdel{de creencias estrechas entre los individuos}\DIFdelend \DIFaddbegin \DIFadd{cuyos individuos tienen un conjunto de creencias estrechamente 
  relacionados}\DIFaddend . Cada soluci\'on (part\'icula) es un ave en el espacio de b\'usqueda que est\'a 
  ...
  \begin{itemize}
   ...
   \item \textbf{\textit{gBest}}, mejor valor objetivo encontrado por las part\'iculas del \DIFdelbegin \DIFdel{c\'umulo}\DIFdelend \DIFaddbegin \DIFadd{\textit{Swarm}}\DIFaddend . Este valor est\'a presente si se 
  ...
  \end{itemize}
    ...
    y el de \DIFdelbegin \DIFdel{velocidad }\DIFdelend \DIFaddbegin \DIFadd{velocidades }\DIFaddend $\vec{v}$, los cuales son sumados para obtener la nueva direcci\'on. Justamente estos cambios de trayectoria 
    ...
    M\'as espec\'ificamente, en cada generaci\'on o iteraci\'on, la \DIFdelbegin \DIFdel{ubicaci\'on }\DIFdelend \DIFaddbegin \DIFadd{direcci\'on }\DIFaddend de cada individuo o part\'icula es afectado por el movimiento 
    ...
    \DIFaddbegin \DIFadd{(}\DIFaddend $\vec{x}'$\DIFaddbegin \DIFadd{) de la part\'icula $\vec{x}$ }\DIFaddend como consecuencia de la suma de la velocidad y los valores adicionales.
	...
	\includegraphics[scale=0.50]{Cap2/1-5.eps}
     ...
     \DIFdelbegin \DIFdel{La ecuacion (\ref{equ:position}) muestra de }\DIFdelend \DIFaddbegin \DIFadd{De }\DIFaddend manera m\'as formal\DIFdelbegin \DIFdel{como }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend una part\'icula se mueve desde una posici\'on del espacio de b\'usqueda hasta otra, simplemente, a\~nadiendo 
     al vector posici\'on \DIFdelbegin \DIFdel{$\vec{x}^t_i$ }\DIFdelend \DIFaddbegin \DIFadd{$x^t_i$ y }\DIFaddend el vector velocidad \DIFdelbegin \DIFdel{$\vec{v}^t_i$ }\DIFdelend \DIFaddbegin \DIFadd{$v^t_i$ }\DIFaddend para obtener un nuevo vector posici\'on, en la 
     generaci\'on $t$ \DIFaddbegin \DIFadd{(ver ecuaci\'on (\ref{equ:position}))}\DIFaddend :

      \begin{equation}
	  \DIFdelbegin %DIFDELCMD < \vec{x}%%%
\DIFdel{^{t+1}_i \leftarrow  }%DIFDELCMD < \vec{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{x^{t+1}_i = x}\DIFaddend ^t_i + \DIFdelbegin %DIFDELCMD < \vec{v}%%%
\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend ^t_i
      ...
      \begin{equation}
	  \DIFdelbegin %DIFDELCMD < \vec{v}%%%
\DIFdel{^{t+1}_{i} \leftarrow }%DIFDELCMD < \vec{v}%%%
\DIFdelend \DIFaddbegin \DIFadd{v^{t+1}_{i} = v}\DIFaddend ^t_i + \phi_1 \cdot rnd_1 \cdot \left(\DIFdelbegin %DIFDELCMD < \vec{pBest}%%%
\DIFdelend \DIFaddbegin \DIFadd{pBest}\DIFaddend ^t_i - \DIFdelbegin %DIFDELCMD < \vec{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend ^t_i \right) 
					    + \phi_2 \cdot rnd_2 \cdot \left(\DIFdelbegin %DIFDELCMD < \vec{g}%%%
\DIFdelend \DIFaddbegin \DIFadd{g}\DIFaddend ^t_i - \DIFdelbegin %DIFDELCMD < \vec{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend ^t_i \right) 
      \label{equ:velocity}
      \end{equation}

      La ecuaci\'on (\ref{equ:velocity}) actualiza el vector velocidad de cada part\'icula $i$ en cada generaci\'on $t$. El componente
      cognitivo est\'a modelado por el factor \DIFdelbegin \DIFdel{$\phi_1 \cdot rnd_1 \cdot \left(\vec{pBest}_i - \vec{x}^t_i \right)$ }\DIFdelend \DIFaddbegin \DIFadd{$\phi_1 \cdot rnd_1 \cdot \left(pBest_i - x^t_i \right)$ }\DIFaddend y representa la distancia entre la 
      posici\'on actual y la mejor conocida por esa part\'icula, es decir, la decisi\'on que tomar\'a la part\'icula influenciada por 
      su propia experiencia a lo largo de su vida. El componente social est\'a modelado por \DIFdelbegin \DIFdel{$\phi_2 \cdot rnd_2 \cdot \left(\vec{g}_i - \vec{x}^t_i \right)$ 
      }\DIFdelend \DIFaddbegin \DIFadd{$\phi_2 \cdot rnd_2 \cdot \left(g_i - x^t_i \right)$ 
    ...
    las posiciones, se calcula el valor de la funci\'on objetivo de cada una y se actualizan los valores iniciales de \DIFdelbegin \DIFdel{$\vec{x}^t$ y $\vec{pBest}^t$}\DIFdelend \DIFaddbegin \DIFadd{$x^t$ y $pBest^t$ }\DIFaddend , en la 
    ...
    maximizaci\'on. Por conveniencia se representa la mejor part\'icula del vecindario como \DIFdelbegin \DIFdel{$\vec{g}^t$}\DIFdelend \DIFaddbegin \DIFadd{$g^t$ }\DIFaddend , pudiendo ser \'esta \DIFdelbegin \DIFdel{$\vec{gBest}^t$ o 
    $\vec{lBest}^t$ }\DIFdelend \DIFaddbegin \DIFadd{$gBest^t$ o 
    $lBest^t$ }\DIFaddend si se consideran vecindarios.

     \begin{figure}
	\centering
	\includegraphics[scale=0.50]{Cap2/1-10.eps}
	  \caption[Ejemplo de la inicializaci\'on del c\'umulo]{Ejemplo de la inicializaci\'on del c\'umulo. Las part\'iculas se mueven \DIFaddbeginFL \DIFaddFL{dentro 
	  }\DIFaddendFL a trav\'es de un proceso iterativo.}
   ...
   % \algsetup{indent=2em}
	...
	\STATE Iniciar el contador de \DIFdelbegin \DIFdel{generaci\'on }\DIFdelend \DIFaddbegin \DIFadd{generacion }\DIFaddend es $t=0$.
	\STATE Iniciar aleatoriamente las posiciones \DIFdelbegin \DIFdel{$\vec{x}_i$ y $\vec{v}_i$ }\DIFdelend \DIFaddbegin \DIFadd{$x_i$ y $v_i$ }\DIFaddend de las $n$ part\'iculas del c\'umulo $S$.
	\STATE Evaluar la funci\'on objetivo con las posiciones $x^{t}$.
	\STATE Encontrar \DIFdelbegin \DIFdel{$\vec{pBest}^t$}\DIFdelend \DIFaddbegin \DIFadd{$pBest^t$ (minimizaci\'on o maximizaci\'on)}\DIFaddend . 
	\STATE Encontrar \DIFdelbegin \DIFdel{$\vec{g}^t$}\DIFdelend \DIFaddbegin \DIFadd{$g^t$ (minimizaci\'on o maximizaci\'on)}\DIFaddend .
	\WHILE {$t < maxG$}
		\FOR {\DIFdelbegin \DIFdel{$i \leftarrow 1$ }\DIFdelend \DIFaddbegin \DIFadd{$i=1$ }\DIFaddend hasta $size(S)$}
		  \STATE Generar una nueva velocidad.
		  \\ \DIFdelbegin \DIFdel{$\vec{v}^{t+1}_{i}\leftarrow \vec{v}^{t}_{i}+\phi_{1}\cdot rnd_{1} \cdot \left(\vec{pBest}^{t}-x^{t}_{i}\right) 
				            +\phi_{2}\cdot rnd_{2} \cdot \left(\vec{g}^{t}-\vec{x}^{t}_{i}\right)$
		  }\DIFdelend \DIFaddbegin \DIFadd{$v^{t+1}_{i}=v^{t}_{i}+\phi_{1}\cdot rnd_{1} \cdot \left(pBest^{t}-x^{t}_{i}\right) 
				            +\phi_{2}\cdot rnd_{2} \cdot \left(g^{t}-x^{t}_{i}\right)$
		  }\DIFaddend \STATE Calcular las nuevas posiciones. 
		  \\\DIFdelbegin \DIFdel{$\vec{x}^{t+1}_{i} \leftarrow \vec{x}^{t}_{i}+\vec{v}^{t+1}_{i}$
		  }\DIFdelend \DIFaddbegin \DIFadd{$x^{t+1}_{i}=x^{t}_{i}+v^{t+1}_{i}$
		  }\DIFaddend \STATE Evaluar la funci\'on objetivo con las nuevas posiciones \DIFdelbegin \DIFdel{$\vec{x}^{t+1}$}\DIFdelend \DIFaddbegin \DIFadd{$x^{t+1}$}\DIFaddend .
		\ENDFOR
		\STATE Encontrar \DIFdelbegin \DIFdel{$\vec{pBest}^t$}\DIFdelend \DIFaddbegin \DIFadd{$pBest^t$ (minimizaci\'on o maximizaci\'on)}\DIFaddend . 
		\STATE Encontrar \DIFdelbegin \DIFdel{$\vec{g}^t$}\DIFdelend \DIFaddbegin \DIFadd{$g^t$ (minimizaci\'on o maximizaci\'on)}\DIFaddend .
		\STATE $t=t+1$.
	...
	part\'icula de la poblaci\'on un valor aleatorio. La segunda es la \DIFaddbegin \DIFadd{parte }\DIFaddend \textit{b\'usqueda} en la cual, el c\'umulo recorre el espacio
  ...
  \begin{itemize}
   ...
   \item \textbf{Circular o Anular}\DIFdelbegin %DIFDELCMD < \cite{JKennedySI}%%%
\DIFdelend : cada individuo est\'a conectado con sus $n$ vecinos inmediatos, intentando imitar al mejor de ellos. En este 
   ...
   \item \textbf{Arcos aleatorios}\DIFdelbegin %DIFDELCMD < \cite{cagnia10}%%%
\DIFdelend : para $n$ part\'iculas se asignan $n$ conexiones sim\'etricas entre pares de individuos. Como su nombre 
   ...
   \item \textbf{Pir\'amide}\DIFdelbegin %DIFDELCMD < \cite{cagnia10} %%%
\DIFdelend : la idea de esta topolog\'ia es la creaci\'on de una estructura tridimensional que comunique a las part\'iculas. 
   ...
   \item \textbf{\'Arbol}\DIFdelbegin %DIFDELCMD < \cite{JKennedySI}%%%
...
\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend ^t_i = \{
	\begin{matrix} 
	  \DIFdelbegin %DIFDELCMD < \vec{v}%%%
\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend ^{t+1}_i & \mbox{si}& \DIFdelbegin %DIFDELCMD < \vec{v}%%%
\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend ^{t+1}_i < \DIFdelbegin %DIFDELCMD < \vec{v}%%%
\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend _{\max}\\
	  \DIFdelbegin %DIFDELCMD < \vec{v}%%%
\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend _{\max} & \mbox{si}& \DIFdelbegin %DIFDELCMD < \vec{v}%%%
\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend ^{t+1}_i \geq \DIFdelbegin %DIFDELCMD < \vec{v}%%%
\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend _{\max}
	\end{matrix}
    \label{equ:control}
    \end{equation}
  ...
  \DIFdelbegin %DIFDELCMD < {\setlength{\parindent}{0pt}
%DIFDELCMD <   %%%
\DIFdel{donde $\vec{v}^{t}_i$ }\DIFdelend \DIFaddbegin \DIFadd{donde $v^{t}_i$ }\DIFaddend es la nueva velocidad, definida como: \DIFdelbegin \DIFdel{$\vec{v}^{t+1}_i$ }\DIFdelend \DIFaddbegin \DIFadd{$v^{t+1}_i$ }\DIFaddend (calculada con la ecuaci\'on (\ref{equ:velocity})) o \DIFdelbegin \DIFdel{$\vec{v}_{max}$}\DIFdelend \DIFaddbegin \DIFadd{$V_{max}$}\DIFaddend , dependiendo de 
  la condici\'on. El valor de \DIFdelbegin \DIFdel{$\vec{v}_{max}$ }\DIFdelend \DIFaddbegin \DIFadd{$V_{max}$ }\DIFaddend nos permite controlar la granularidad de la b\'usqueda escalando las velocidades. Valores 
...
\DIFdelend \DIFaddbegin 
  ...
  El factor de Constricci\'on \DIFaddbegin \DIFadd{fue presentado por }\cite{clerc99}\DIFadd{. Este factor }\DIFaddend consta de un conjunto de ecuaciones lineales que 
  derivan en la convergencia \'optima de las part\'iculas. En este modelo, la velocidad est\'a restringida por una
  constante $\chi$, con el objeto de asegurar la convergencia a un punto estable del espacio\DIFdelbegin %DIFDELCMD < \cite{clerc99}%%%
\DIFdelend . La ecuaci\'on (\ref{equ:velocity})
  de velocidad se reemplaza por la (\ref{equ:const}).

    \begin{equation}
	  \DIFdelbegin %DIFDELCMD < \vec{v}%%%
\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend ^{t+1}_{i} = \chi \left( \DIFdelbegin %DIFDELCMD < \vec{v}%%%
\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend ^t_i + \phi_1 \cdot rnd_1 \cdot \left(\DIFdelbegin %DIFDELCMD < \vec{pBest}%%%
\DIFdelend \DIFaddbegin \DIFadd{pBest}\DIFaddend ^t_i - \DIFdelbegin %DIFDELCMD < \vec{x}%%%
\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend ^t_i \right) 
					    + \phi_2 \cdot rnd_2 \cdot \left(\DIFdelbegin %DIFDELCMD < \vec{g}%%%
\DIFdelend \DIFaddbegin \DIFadd{g}\DIFaddend ^t_i - \DIFdelbegin %DIFDELCMD < \vec{x}%%%
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{siendo
...
\DIFdelend \DIFaddbegin 
    ...
    posiciones (ecuaci\'on \ref{equ:position}) permanece sin cambios en esta variante.
     ...
     \begin{equation}
	  \DIFdelbegin %DIFDELCMD < \vec{v}%%%
\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend ^{t+1}_{i} = \omega \cdot \DIFdelbegin %DIFDELCMD < \vec{v}%%%
\DIFdelend \DIFaddbegin \DIFadd{v}\DIFaddend ^t_i + \phi_1 \cdot rnd_1 \cdot \left(\DIFdelbegin %DIFDELCMD < \vec{pBest}%%%
\DIFdelend \DIFaddbegin \DIFadd{pBest}\DIFaddend ^t_i - \DIFdelbegin %DIFDELCMD < \vec{x}%%%
...
\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend ^t_i \right) 
    ...
    \begin{itemize}
     ...
     \item \textbf{Modelo social exclusivo}: $\phi_1 = 0$, $\phi_2 > 0$ y \DIFdelbegin \DIFdel{$\vec{g} = \vec{x}_i$}\DIFdelend \DIFaddbegin \DIFadd{$g = x_i$}\DIFaddend . La posici\'on de la part\'icula en s\'i no puede 
    ...
    que indica \DIFaddbegin \DIFadd{incluso mejor }\DIFaddend elegir un factor m\'as cognitivo ($\phi_1$) que un factor social ($\phi_2$), con $\phi_1 + \phi_2 < 4$ 
  ...
  vecindarios) para el caso de optimizaci\'on \DIFdelbegin \DIFdel{multi-objetivo}\DIFdelend \DIFaddbegin \DIFadd{multiobjetivo}\DIFaddend . En los problemas de optimizaci\'on multi-objetivo, todas las soluciones 
  ...
  %\procedure{asa}{asa}
	\REQUIRE Problema de Optimizaci\'on.
	\ENSURE Soluciones no dominadas en un archivo acotado $A$.
		\STATE Iniciar contador de generaciones, \DIFdelbegin \DIFdel{$t \leftarrow 0$
		}\DIFdelend \DIFaddbegin \DIFadd{$t=0$
		}\DIFaddend \STATE Iniciar contador (\DIFdelbegin \DIFdel{$C\leftarrow 0$}\DIFdelend \DIFaddbegin \DIFadd{$C=0$}\DIFaddend ) de part\'iculas no dominadas en el archivo $A$
		\STATE Iniciar aleatoriamente las posiciones \DIFdelbegin \DIFdel{$\vec{x}^{0}_{i}$ }\DIFdelend \DIFaddbegin \DIFadd{$x^{0}_{i}$ }\DIFaddend de la poblaci\'on, donde $i=\left\{0,\ldots,n\right\}$.		
		\STATE Iniciar la velocidad \DIFdelbegin \DIFdel{$\vec{v}^{0}_{i} = 0$ }\DIFdelend \DIFaddbegin \DIFadd{$v^{0}_{i} = 0$ }\DIFaddend de la poblaci\'on, donde $i=\left\{0,\ldots,n\right\}$.
	...
	\WHILE {$t < maxG$}									
			...
			\STATE Calcular la nueva velocidad y posici\'on de cada part\'icula en la poblaci\'on.
			  \FOR {\DIFdelbegin \DIFdel{$i\leftarrow 1$ }\DIFdelend \DIFaddbegin \DIFadd{$i=1$ }\DIFaddend hasta $size(S)$}
			    \STATE Generar una nueva velocidad 
			      \\ \DIFdelbegin \DIFdel{$\vec{v}^{t+1}_{i}\leftarrow\omega\cdot \vec{v}^{t}_{i}+\phi_{1}\cdot rnd_{1} \cdot \left(\vec{pBest}^{t}_{i}-
			      \vec{x}^{k}_{i}\right) 
			      +\phi_{2}\cdot rnd_{2} \cdot \left(\vec{g}^{t}_{i}-\vec{x}^{k}_{i}\right)$
			    }\DIFdelend \DIFaddbegin \DIFadd{$v^{t+1}_{i}=\omega\cdot v^{t}_{i}+\phi_{1}\cdot rnd_{1} \cdot \left(pBest^{t}_{i}-x^{k}_{i}\right) 
...
\section{Hipervolumen}
  ...
  estudios que analizan el uso de esta medida para la b\'usqueda \DIFdelbegin \DIFdel{multi-objetivo }\DIFdelend \DIFaddbegin \DIFadd{multiobjetivo }\DIFaddend y varios m\'as sobre m\'etodos eficientes para 
  el c\'alculo \DIFdelbegin \DIFdel{del }\DIFdelend hipervolumen \cite{FonPaqhypervolume, BeuFon2009, Emmerich05anemo}
 ...
 \begin{figure}
  ...
  \begin{algorithmic}[1]
   %   \procedure {qwqw}{qwqw}
	\REQUIRE Una poblaci\'on \DIFdelbegin \DIFdel{$\mathcal{P}=\left\{y_1,\ldots,y_n\right\}$}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{P}=\left\{y_1,\ldots,y_n\right\} \subseteq \psi$}\DIFaddend , donde $\mathcal{P}_i = \left(y_{i 1},\ldots,y_{i d}\right) $, 
	...
	\ENDIF
	\STATE \DIFdelbegin \DIFdel{$volume \leftarrow 0$
	}\DIFdelend \DIFaddbegin \DIFadd{$volume = 0$
	}\DIFaddend \STATE $splitCount\left[\right]\Leftarrow n$
	\FOR {\DIFdelbegin \DIFdel{$j\leftarrow 1, n$}\DIFdelend \DIFaddbegin \DIFadd{$j=1, n$}\DIFaddend }
		...
		\ENDFOR
	\ENDFOR
	\DIFdelbegin %DIFDELCMD < \FOR{$i\leftarrow 1, n$}
%DIFDELCMD < 			%%%
\DIFdelend \DIFaddbegin \FOR{$i=1, n$}
			\DIFaddend \STATE $coveredCount\left[\right]\Leftarrow 0$
			\FOR {\DIFdelbegin \DIFdel{$j\leftarrow 1, d$}\DIFdelend \DIFaddbegin \DIFadd{$j=1, d$}\DIFaddend }
				\STATE $coveredCount\left[order\left[i-1\right]\left[j-1\right]\right]++$			
			\ENDFOR
			\DIFdelbegin %DIFDELCMD < \FOR{$k=\leftarrow n-1, 0$}
%DIFDELCMD < 				%%%
\DIFdelend \DIFaddbegin \FOR{$k=n-1, 0$}
			...
			\ENDFOR
	\ENDFOR
	\DIFdelbegin %DIFDELCMD < \FOR{$j\leftarrow 1,d$}
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \FOR{$j=1,d$}
...
\end{algorithm}
  ...
  Este compromiso\DIFaddbegin \DIFadd{, seg\'un }\cite{Bader11}\DIFadd{, }\DIFaddend es aceptable, siempre y cuando los indicadores que aproximan el valor del hipervolumen, a 
  pesar de la p\'erdida de precisi\'on adoptada, siguen siendo capaces de  clasificar y comparar de forma fiable los conjuntos de soluciones\DIFdelbegin %DIFDELCMD < \cite{Bader11}%%%
\DIFdel{. 
  Se han desarrollado m\'etodos }\DIFdelend \DIFaddbegin \DIFadd{. 
  }\cite{Everson02fullelite} \DIFadd{desarrollaron un  m\'etodo }\DIFaddend de muestreo (o \textit{Monte Carlo}) para aproximar el hipervolumen el cual ha 
  demostrado ser un buen estimador, capaz de aumentar la precisi\'on conforme se aumenta el n\'umero de puntos de la muestra\DIFdelbegin %DIFDELCMD < \cite{Everson02fullelite}%%%
\DIFdelend . 
 ...
 % \algsetup{indent=2em}
  \begin{algorithmic}[1]			
    %\procedure{asdf}{sdfsd}
		\REQUIRE Una poblaci\'on \DIFdelbegin \DIFdel{$\mathcal{P}=\left\{y_1,\ldots,y_n\right\}$, 
		donde $\mathcal{P}_i = \left(y_{i,1},\ldots,y_{n,d}\right) $}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{P}=\left\{a_1,\ldots,a_n\right\} \subseteq \psi$, 
		donde $\mathcal{P}_i = \left(a_{i,1},\ldots,a_{n,d}\right) $}\DIFaddend , un punto de referencia \DIFdelbegin \DIFdel{$R=\left(r_1,\ldots,r_n\right)$}\DIFdelend \DIFaddbegin \DIFadd{$r=\left(r_1,\ldots,r_n\right)$}\DIFaddend .		
		\ENSURE Contribuci\'on al hipervolumen $C_{\mathcal{P}}$
		\STATE $C\Leftarrow 0$
		\WHILE{$\forall a \in \mathcal{P}$}
			\STATE \DIFdelbegin \DIFdel{$C_{a} \leftarrow CalcVolume(\mathcal{P},r) - CalcVolume(\mathcal{P}\backslash \left\{a\right\},R)$
		}\DIFdelend \DIFaddbegin \DIFadd{$C_{a} = CalcVolume(\mathcal{P},r) - CalcVolume(\mathcal{P}\backslash \left\{a\right\},r)$
		}\DIFaddend \ENDWHILE
		\RETURN $C$
%	\EndProcedure
  \end{algorithmic}
  ...
  \end{algorithm}
   ...
   descartar.
    ...
    HypE es otro algoritmo basado en muestreo para aproximar \DIFdelbegin \DIFdel{las }\DIFdelend \DIFaddbegin \DIFadd{la }\DIFaddend contribuciones al hipervolumen de un conjunto de 
  ...
  de $a$, denotada como $C^k_h\left(a,\mathcal{P},\mathcal{R} \right)$, para todos los elementos \DIFdelbegin \DIFdel{de }\DIFdelend $a \in \mathcal{P}$ con 
...
\begin{algorithm}
\begin{algorithmic}[1]	
      \REQUIRE \DIFdelbegin \DIFdel{$\mathcal{P}=\left\{y_1,\ldots,y_n\right\}$, donde $\mathcal{P}_i = \left(y_{i,1},\ldots,y_{n,d}\right) $ , 
		$\varGamma = \bigcup_{a \in \mathcal{P}} \left\{ \left(a, 0 \right) \right\}$,
		un punto de referencia $R=\left(r_1,\ldots,r_n\right)$}\DIFdelend \DIFaddbegin \DIFadd{Una poblaci\'on $\mathcal{P} \subset \psi$, donde $F= \bigcup_{a \in \mathcal{P}} \left\{ \left(a, 0 \right) \right\}$}\\
      \DIFadd{Un conjunto de referencia $R \subset Z$}\DIFaddend , un par\'ametro \DIFdelbegin \DIFdel{$k \in \mathds{N}$}\DIFdelend \DIFaddbegin \DIFadd{$k \in N$}\DIFaddend \\
      \ENSURE Contribuci\'on del hipervolumen $C_{\mathcal{P}}$
	\STATE \textit{Se filtran las soluciones relevantes} \DIFdelbegin \DIFdel{$A_U \leftarrow \bigcup_{\left(a,v\right)\in \varGamma, \forall i<j\leq d:f_{j}\left(a\right)\leq z_{j}}\left\{f\left(a\right)\right\} $ 
	}\DIFdelend \DIFaddbegin \DIFadd{$A_U \leftarrow \bigcup_{\left(a,v\right)\in F, \forall i<j\leq d:f_{j}\left(a\right)\leq z_{j}}\left\{f\left(a\right)\right\} $ 
	}\DIFaddend \STATE \textit{Se filtran los puntos de referencia} \DIFdelbegin \DIFdel{$U_R \leftarrow \bigcup_{\left(r_1,\ldots,r_d\right)\in R, \forall i<j\leq d:r_{j}\geq z_{j}} \left\{\left(r_1,\ldots,r_d\right)\right\}$ 
	}\DIFdelend \DIFaddbegin \DIFadd{$UR \leftarrow \bigcup_{\left(r_1,\ldots,r_d\right)\in R, \forall i<j\leq d:r_{j}\geq z_{j}} \left\{\left(r_1,\ldots,r_d\right)\right\}$ 
	}\DIFaddend \IF {\DIFdelbegin \DIFdel{$i=0 \wedge U_R \ != 0$}\DIFdelend \DIFaddbegin \DIFadd{$i=0 \wedge UR \ != 0$}\DIFaddend }			
		\STATE \DIFdelbegin \DIFdel{$\alpha \leftarrow \prod^{\left|A_{U}\right|-1}_{j=1} \left(k-j\right)/\left(\left|\varGamma\right| - j\right)$}\DIFdelend \DIFaddbegin \DIFadd{$\alpha \leftarrow \prod^{\left|A_{U}\right|-1}_{j=1} \left(k-j\right)/\left(\left|F\right| - j\right)$}\DIFaddend .			
		\STATE \DIFdelbegin \DIFdel{$\varGamma^{'} \leftarrow 0$
		}\DIFdelend \DIFaddbegin \DIFadd{$F^{'} \leftarrow 0$
		}\DIFaddend \FOR {\DIFdelbegin \DIFdel{$\left(a,v\right)\in \varGamma $}\DIFdelend \DIFaddbegin \DIFadd{$\left(a,v\right)\in F $}\DIFaddend }
			\IF { $\forall 1 \leq j \leq d: f_{j} \left(a\right)\leq z_{j}$}
				\STATE \DIFdelbegin \DIFdel{$\varGamma^{'} \leftarrow \varGamma^{'} \cup \left\{\left(a,v + \frac{\alpha}{\left|A_{u}\right|}\cdot V\right)\right\}$
			}\DIFdelend \DIFaddbegin \DIFadd{$F^{'} \leftarrow F^{'} \cup \left\{\left(a,v + \frac{\alpha}{\left|A_{u}\right|}\cdot V\right)\right\}$
			}\DIFaddend \ELSE 
				\STATE \textit{Se actualiza el hipervolumen de las soluciones filtradas}\\
				\DIFdelbegin \DIFdel{$\varGamma^{'} \leftarrow \varGamma^{'} \cup \left\{\left(a,v\right)\right\}$
			}\DIFdelend \DIFaddbegin \DIFadd{$F^{'} \leftarrow F^{'} \cup \left\{\left(a,v\right)\right\}$
			}\DIFaddend \ENDIF
		\ENDFOR
	\ELSE \IF{$i>0$} 
		\STATE \DIFdelbegin \DIFdel{$\varGamma^{'} \leftarrow \varGamma$ 
		}\DIFdelend \DIFaddbegin \DIFadd{$F^{'} \leftarrow F$ 
		}\DIFaddend \STATE \DIFdelbegin \DIFdel{$U \leftarrow A_{U} \cup U_R$ }\DIFdelend \DIFaddbegin \DIFadd{$U \leftarrow A_{U} \cup UR$ }\DIFaddend \\
		...
		\WHILE {$U^{'} \ != \emptyset$} 
			...
			\IF {$U' \ != \emptyset$}
				...
				\STATE \DIFdelbegin \DIFdel{$\varGamma^{'} \leftarrow hypeExact \left(\varGamma^{'},R,k,i-1,V^{'},\left(z_{1},\ldots,z_{i-1},u^{*},z_{i+1},\ldots,z_{d}\right)\right)$
			}\DIFdelend \DIFaddbegin \DIFadd{$F^{'} \leftarrow hypeExact \left(F^{'},R,k,i-1,V^{'},\left(z_{1},\ldots,z_{i-1},u^{*},z_{i+1},\ldots,z_{d}\right)\right)$
			}\DIFaddend \ENDIF 
			\STATE \DIFdelbegin \DIFdel{$U \leftarrow U^{'}$
		}\DIFdelend \DIFaddbegin \DIFadd{$U=U^{'}$
		}\DIFaddend \ENDWHILE
		\ENDIF
	\ENDIF		
	\STATE \DIFdelbegin \DIFdel{$C \leftarrow \varGamma^{'} $
	}\DIFdelend \DIFaddbegin \DIFadd{$C = F^{'} $
	}\DIFaddend \RETURN  $C$
  %    \EndProcedure
  \end{algorithmic}
\caption[$hypeExact$]{\DIFdelbegin \DIFdel{$hypeExact \left(\mathcal{P},R,k,i,V,\left(z_{1},\ldots,z_{d}\right)\right)$}\DIFdelend \DIFaddbegin \DIFadd{$hypeExact \left(F,R,k,i,V,\left(z_{1},\ldots,z_{d}\right)\right)$}\DIFaddend . C\'alculo exacto de la contribuci\'on del Hipervolumen}
...
la poblaci\'on es un conjunto m\'ultiple, es decir, puede contener soluciones \DIFaddbegin \DIFadd{indiferentes o }\DIFaddend duplicadas. Por lo tanto, todos los 
...
que \DIFdelbegin \DIFdel{una }\DIFdelend \DIFaddbegin \DIFadd{unas }\DIFaddend de las aplicaciones m\'as comunes del m\'etodo de Monte Carlo es el c\'alculo de integrales, \'este es una 
...
\begin{algorithm}
\begin{algorithmic}[1]
	\REQUIRE \DIFdelbegin \DIFdel{$\mathcal{P}=\left\{y_1,\ldots,y_n\right\}$, donde $\mathcal{P}_i = \left(y_{i,1},\ldots,y_{n,d}\right)$, un punto de referencia $R=\left(r_1,\ldots,r_n\right)$}\DIFdelend \DIFaddbegin \DIFadd{Una poblaci\'on $\mathcal{P} \subset \psi$, un conjunto de referencia $\mathcal{R} \subset Z$}\DIFaddend , un par\'ametro \DIFdelbegin \DIFdel{$k \in \mathds{N}$ y }\DIFdelend \DIFaddbegin \DIFadd{$k \in N$, 
	}\DIFaddend un n\'umero de muestras \DIFdelbegin \DIFdel{$M \in \mathds{N}$		
		}\DIFdelend \DIFaddbegin \DIFadd{$M \in N$		
		}\DIFaddend \FOR {$i\leftarrow 1,n$}
			\STATE \DIFdelbegin \DIFdel{$l_i \leftarrow min_{a \in \mathcal{P}}f_{i} \left(a\right)$
			}\DIFdelend \DIFaddbegin \DIFadd{$l_i = min_{a \in \mathcal{P}}f_{i} \left(a\right)$
			}\DIFaddend \STATE \DIFdelbegin \DIFdel{$U_i \leftarrow max_{\left(r_1.\ldots,r_d\right)\in \mathcal{R}}r_i$
		}\DIFdelend \DIFaddbegin \DIFadd{$U_i = max_{\left(r_1.\ldots,r_d\right)\in \mathcal{R}}r_i$
		}\DIFaddend \ENDFOR
		...
		\STATE \DIFdelbegin \DIFdel{$\varGamma \leftarrow \bigcup_{a \in \mathcal{P}}\left\{\left(a,0\right)\right\}$		
		}\DIFdelend \DIFaddbegin \DIFadd{$F \leftarrow \bigcup_{a \in \mathcal{P}}\left\{\left(a,0\right)\right\}$		
			...
			\IF {$\exists r\in \mathcal{R}: s \leq r$}
					...
					\STATE \DIFdelbegin \DIFdel{$\varGamma^{'}\leftarrow 0$
						...
						\IF {$f\left(a\right)\leq s$}
							\STATE \DIFdelbegin \DIFdel{$\varGamma^{'} \leftarrow \varGamma^{'} \cup \left\{\left(a, v + \frac{\alpha}{\left|A_U\right|}\cdot \frac{V}{m}\right)\right\}$						
						}\DIFdelend \DIFaddbegin \DIFadd{$F^{'} \leftarrow F^{'} \cup \left\{\left(a, v + \frac{\alpha}{\left|A_U\right|}\cdot \frac{V}{m}\right)\right\}$						
						}\DIFaddend \ELSE
							\STATE \DIFdelbegin \DIFdel{$\varGamma^{'} \leftarrow \varGamma^{'} \cup \left\{\left(a, v \right)\right\}$						
						}\DIFdelend \DIFaddbegin \DIFadd{$F^{'} \leftarrow F^{'} \cup \left\{\left(a, v \right)\right\}$						
						}\DIFaddend \ENDIF						
					\ENDFOR
						\STATE \DIFdelbegin \DIFdel{$\varGamma^{'} \leftarrow \varGamma$						
			...
			\ENDIF 
		\ENDFOR
	\STATE \DIFdelbegin \DIFdel{$C = \varGamma^{'} $
	}\DIFdelend \DIFaddbegin \DIFadd{$C = F^{'} $
	}\DIFaddend \RETURN $C$
\end{algorithmic}
\caption[$hypeSampling$]{\DIFdelbegin \DIFdel{$hypeSampling \left(\mathcal{P},R,k,i,V,\left(z_{1},\ldots,z_{d}\right),M\right)$}\DIFdelend \DIFaddbegin \DIFadd{$hypeSampling \left(F,R,k,i,V,\left(z_{1},\ldots,z_{d}\right),M\right)$}\DIFaddend . Estimando el c\'alculo de la contribuci\'on del Hipervolumen}
\label{alg:hvaprox} 
\end{algorithm}
...
\DIFdelend 
  ...
  La manera de hacerlo es seleccionar al azar de manera uniforme \DIFdelbegin \DIFdel{varios }\DIFdelend \DIFaddbegin \DIFadd{$m$ }\DIFaddend objetivos ($s_1, \ldots, s_m$) de la caja de muestreo $S$. 
...
%DIFDELCMD <   {\setlength{\parindent}{0pt}
%DIFDELCMD <   %%%
\DIFdel{donde $\alpha_i = \prod^{i-1}_{l=1} \frac{k-l}{\left|\mathcal{P}\right|-l}$.
  }%DIFDELCMD < }
%DIFDELCMD <     %%%
\DIFdelend 
